...
const int MAX_SIZE_JACOBI = 10; // Maksimum ukuran SPL

// Fungsi untuk mencetak matriks
void printMatrixJacobi(double matrix[MAX_SIZE_JACOBI][MAX_SIZE_JACOBI], int n) {
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            cout << matrix[i][j] << " ";
        }
...
void jacobiMethod(double matrix[MAX_SIZE_JACOBI][MAX_SIZE_JACOBI], double constants[MAX_SIZE_JACOBI], double solution[MAX_SIZE_JACOBI], int n, double tolerance, int maxIterations) {
    double x[MAX_SIZE_JACOBI]; // Solusi iterasi sebelumnya
...
int mainJacobi() {
    ...
    double matrix[MAX_SIZE_JACOBI][MAX_SIZE_JACOBI]; // Matriks koefisien SPL
    double constants[MAX_SIZE_JACOBI];        // Vektor konstanta
    ...
    double solution[MAX_SIZE_JACOBI]; // Solusi SPL
